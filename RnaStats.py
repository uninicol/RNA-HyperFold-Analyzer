...
class RnaStats:
    def __init__(self, forna_file_path):
        ...
        self.__precomputed_H = None

    def plot_hypergraph(self, size=(40, 40)):
    ...
    def __get_hyperedges(self, node):
        return [edge for edge in self.H.edges if node in self.H.edges[edge]]

    def get_subset_conductance(self, subset):
    ...
    def get_partitions_conductance(self):
        return enumerate(
            [self.get_subset_conductance(subset) for subset in self.partitions()]
        )

    def plot_partitions_conductance(self):
    ...
    def get_n_between_centrality(self, n=1):
        return hnx.algorithms.s_betweenness_centrality(self.H, n)

    def plot_n_between_centrality(self, n=1):
